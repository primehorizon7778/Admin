...
function initGlobalListeners() {
    // 1. Fetch Dynamic Admin Credentials from Firebase
    ...
    // 2. Track Admin Session Expiry (Remote Logout)
    ...
    // 3. Track All Queues for Global Top Blinking Alert
...
function checkAdminLogin() {
    ...
    // Slight delay to allow Firebase to fetch live credentials
    setTimeout(() => {
        ...
        if (isValid) {
            ...
            forceLocalLogout(false); // Clean up variables and show login
        }
    }, 800);
}
...
function initAdminApp() {
    ...
    // Load Settings
    db.ref('admin_settings/min_withdraw').once('value', s => { if (s.exists()) document.getElementById('min-withdraw').value = s.val(); });
    db.ref('admin_settings/coins_per_rupee').once('value', s => { if (s.exists()) document.getElementById('coins-per-rupee').value = s.val(); });
...
function showSection(section) {
    ...
    if (title) {
        const titles = { 'sessions': 'Session Phone', 'kyc': 'KYC Approvals', 'doubleauth': 'Double Verification', 'password': 'Password Recovery', 'active-verified': 'Verified & Active Nodes', 'global-logs': 'Global System Logs' };
...
function renderUserCard(user, container) {
    ...
    const freeWithdraws = user.freeWithdraws !== undefined ? user.freeWithdraws : 1; // Free withdraw tracking
...
function loadActiveVerified() {
    ...
    db.ref('users').on('value', snap => {
        ...
        snap.forEach(child => {
            const u = child.val();
            // Sirf wo user jin ka plan active ho
            if(u.activePlan) {
                count++;
                ...
                // Check double verification
                let is2FA = u.doubleAuthEnabled === true;
                
                // Set badges properly based on 2FA status
                let badgeHtml = is2FA 
                ...
                // Render Turn OFF 2FA option only if it is actually ON
...
function loadDepositsAdvanced() {
    ...
    db.ref('admin_queues/deposits').on('value', snap => {
        ...
        snap.forEach(child => {
            ...
            hasPending = true;
            const div = document.createElement('div');
            div.className = 'glass p-5 rounded-3xl flex flex-col md:flex-row gap-5 items-center border border-emerald-500/30';
            div.innerHTML = `
                <div class="flex-1 min-w-0">
                    <div class="font-extrabold text-lg">${d.username || 'Unknown User'}</div>
                    <div class="text-emerald-400 font-mono text-sm">${d.userPhone}</div>
                    <div class="mt-3"><span class="text-xs text-slate-400">AMOUNT</span><br><span class="font-black text-3xl">Rs. ${d.amount}</span></div>
                    <div class="text-xs text-slate-400 mt-2">TRX ID: <span class="font-mono text-white">${d.trxId || 'N/A'}</span></div>
                </div>
...
function loadWithdrawalsAdvanced() {
    ...
    db.ref('admin_queues/withdrawals').on('value', snap => {
        ...
        snap.forEach(child => {
            ...
            const div = document.createElement('div');
            div.className = 'glass p-5 rounded-3xl border border-purple-500/30 flex flex-col md:flex-row gap-4 items-start md:items-center justify-between';
            div.innerHTML = `
                <div class="flex-1">
                    <div class="font-extrabold text-lg">${w.username || 'Unknown User'}</div>
                    <div class="text-sm text-purple-400 font-mono">${w.userPhone}</div>
                ...
                <div class="text-right w-full md:w-auto">
                    <div><span class="text-xs text-slate-400">REQUESTED</span><br><span class="font-black text-3xl text-purple-400">Rs. ${requestedAmount}</span></div>
...
async function renderUserHistory(phone, container) {
    ...
    if(depSnap.exists()) depSnap.forEach(c => { const d=c.val(); if(d.userPhone===phone) { if(d.status==='Approved') totalD+=Number(d.amount); timeline.push({type:'Deposit', id:c.key, amount:d.amount, status:d.status, ts:d.timestamp}); } });
    if(wSnap.exists()) wSnap.forEach(c => { const w=c.val(); if(w.userPhone===phone) { if(w.status==='Approved') totalW+=Number(w.amount); timeline.push({type:'Withdraw', id:c.key, amount:w.amount, status:w.status, ts:w.timestamp}); } });
    ...
    timeline.forEach(item => {
        const div = document.createElement('div');
        div.className = 'flex items-start gap-4 p-4 border-b border-slate-700 last:border-0';
        div.innerHTML = `<div class="flex-1"><div class="font-extrabold">${item.type} <span class="text-xs text-slate-400 font-mono">(${item.id})</span></div><div class="text-sm mt-1">${item.amount ? `Rs. ${item.amount}` : item.message}</div><div class="text-xs text-slate-500 mt-1">${new Date(item.ts).toLocaleString()} • ${item.status}</div></div>`;
...
function saveLimits() {
    ...
    triggerToast('Limits & Rates saved');
}
